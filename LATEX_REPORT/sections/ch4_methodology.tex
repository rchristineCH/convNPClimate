...
The RBF interpolation layer lets the model predict at arbitrary target points rather than only on the input grid. Downscaling needs exactly this, since the fine target locations do not align with the coarse lattice, and the same property allows the model to be queried directly at station coordinates, as in \textcite{vaughan2022convcnp} or in \cref{ch:stations}.
...
\end{equation}
This gives one mean and one spread per target point and day (\sref{sec:app-distributions} illustrates the resulting density).
...
\end{equation}
which reads directly: the day has to be wet, which costs $\rho$, and the amount then has to reach $t$. The precipitation indices and skill scores later ask exactly this exceedance probability of the model (\sref{subsec:app-skill-precip}). $\rho$ reweights the same wet body without changing its shape such that the gap to 1 at 0 mm is the probability of a dry day. $\beta$ changes how much precipitation accumulates once it does rain, stretching the curve to the right while the dry mass stays where it is. $\alpha$ changes the shape of a wet day: a given average is reached either through many small amounts or through fewer, heavier ones (\sref{sec:app-distributions} visualises these shapes at a single point and day). \sref{subsec:app-skill-precip} explains how these point-day distributions are pooled into the frequencies the results chapter actually reports.
...
No hyperparameters are tuned on the folds, so there is no separate model to refit on the merged span afterwards the way a tune-then-refit workflow would require. The five fold models are trained anyway, to give every training-period day an honest holdout score; this is the CV-holdout score used throughout \cref{ch:single-model} to compare configurations and objectives against each other. At inference, we simply keep all five and ensemble them (\sref{sec:meth-inference}), rather than picking one at random or paying for a sixth training run on the merged span.

The folds are temporal blocks, not independent resamples: they were built that way so a held-out block cannot leak near-duplicate weather from its neighbours (\sref{subsec:meth-cv}), but it also means any two fold models still share three of their four training blocks. That is far more overlap than the independent ensemble members this technique is usually validated on \cite{dutschmann2023kfoldensemble}, so the between-fold spread in \eref{eq:fold-ensemble} likely understates the true disagreement between models rather than measuring it cleanly. The advantage of ensembling is no retraining is needed and still likely reduces prediction noise, even with correlated folds; the disadvantage is that its uncertainty signal is conservative rather than calibrated, and its accuracy against a genuine single model is untested. A model trained on the full 2020--2023 span would add a clean baseline to check that accuracy against, and access to all five blocks of training data at once, rather than the four out of five each fold model sees.

\subsection{Objective Functions}\label{objective}
The negative log-likelihood objectives follow \textcite{vaughan2022convcnp}. For the precipitation model, we extended the training with a CRPS-based objective function, used both from scratch and as a fine-tune of a converged likelihood model. Ten epochs of fine-tuning recover more of the held-out CRPS than thirty epochs of training on it from scratch, which is why the fine-tune counts as its own, third run rather than a variant of the second (\sref{sec:app-precip-training}).
...
The second objective is the CRPS of \eref{eq:crps}, which judges the whole predictive distribution instead of the density at the value that occurred. A closed form for the Bernoulli--Gamma mixture is plausibly derivable, by the same route used for the censored-shifted Gamma distribution of \textcite{scheuerer2015csgd} and for the Gamma component's own closed-form CRPS \cite{gneiting2007scoring}, but we did not pursue this derivation; instead the score is estimated in the kernel form given by \textcite{gneiting2007scoring}, which writes the score as expected absolute differences and therefore needs only draws from the predictive distribution. This choice is simpler to implement and drops in directly wherever the objective is evaluated, including during backpropagation. Applied to the mixture, this leaves the dry outcome analytic and only the two Gamma expectations to sample, each from its own independent set of $25$ draws taken with the implicit reparameterisation of \textcite{figurnov2018implicit}, which carries the gradient to $\alpha$ and $\beta$.
...
\section{Computational Cost}\label{sec:meth-cost}

The model is small in weights and expensive in use. The configurations range from $282\,000$ to
$325\,000$ parameters, $1.1$ to $1.3\,\mathrm{MB}$ in single precision; the cost is set by the tens
of thousands of target points rather than by the network, dominated by the elevation MLP and the
RBF distance matrix (\sref{sec:app-cost}).

In practice a single mid-range GPU is sufficient: at the batch size of $8$ used here the forward
pass peaks below $2\,\mathrm{GB}$, the backward pass raises the full training step to just below
$2.2\,\mathrm{GB}$, and inference roughly halves that, so both fit on consumer hardware, while a
server allows several configurations to be trained at once. Should the cost ever become binding,
the easiest levers are a narrower elevation MLP, an RBF kernel restricted to the grid cells within
a few length scales of each target point, or a distance matrix kept in reduced numerical precision.
...
off its weights, since the encoder mixes all channels immediately after its per-channel smoothing. This section attributes the model's output to its inputs, to see which channels it relies on most and whether that reliance is physically plausible.

\subsection{Choosing the Attribution Method}\label{subsec:meth-fi-choice}
\tref{tab:meth-fi-methods} compares candidate attribution methods against three requirements: the result must describe the trained model as a whole, query the model itself rather than a substitute, and be expressed as a loss of accuracy so models can be compared. Permutation importance with time as the permutation axis meets all three and is the cheapest computationally, so it is used as the primary method, with LIME run as a cross-check (\sref{subsec:app-fi-lime}) and SHAP implemented but not systematically run due to its cost. The estimator's limitations (what a time-invariant channel or a destroyed-accuracy measure can and cannot support) are set out in \sref{subsec:app-fi-limits}.
